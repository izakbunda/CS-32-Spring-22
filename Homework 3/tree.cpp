...
int numberOfTimesHas(const string a1[], int n1, const string a2[], int n2)
{
    ...
    if ( (n1 <= 0) && (n2 != 0) ) { // got to the end of a1 without getting through the end of a2
        return 0;
    }
    if ( a1[0] == a2[0] ) {
...
void split(string a[], int n, string splitter, int& firstNotLess, int& firstGreater)
{
    if (n < 0)
        n = 0;
...
void order(string a[], int n)
{
    ...
    int middle = n/2; // a[middle] will act as the "splitter"
    int fNl, fG;
    
    split( a, n, a[middle], fNl, fG ); // splits up every sub-string in the middle
    
    if ( fNl != n ) {
        order( a, fNl ); // this is for the first half
    }
    if ( fG != n ) {
        order( a+fG, n-fG ); // this is for the second half
    }
}
